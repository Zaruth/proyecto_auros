...
/**
 * Controlador del trabajo de los usuarios
 */
class WorkController extends Controller
{
    ...
    */
    ...
    /**
     * Muestra la pantalla de trabajo
     * @Route("/juego/work", name="work")
     */
    public function index(UserController $uc)
    {
    ...
    /**
     * Comienza el trabajo de un jugador
     * @Route("/juego/work/start", name="start_work", methods="GET|POST")
     */
    public function start_work(Request $request, UserController $uc): Response
    { 
        if($uc->check_ban($this->getUser())){
            return $this->redirectToRoute('logout');
        }else{
    ...
    /**
     * Finaliza un trabajo de un jugador
     * @Route("/juego/work/finish", name="finish_work", methods="GET|POST")
     */
    public function finish_work(Request $request, UserController $uc): Response
    {
        if($uc->check_ban($this->getUser())){
            return $this->redirectToRoute('logout');
        }else{
            ...
            if($diferencia <= 0){
                ...
                $msg = new Message();
                $msg->setText("Has trabajado durante {$hor} horas {$min} minutos y {$sec} segundos.<br/>Has ganado {$exp} puntos de experiencia y 100<img src=\"../../img/game/misc/goldcoin.png\"> monedas de oro.");
